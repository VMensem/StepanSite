...
# Функция получения онлайна сервера
...
# Функция для создания stats, чтобы не дублировать
def get_stats():
...
@app.route('/shop')
def shop():
    return render_template('shop.html', payments=payments, stats=get_stats())

@app.route('/success')
def success():
    return render_template('success.html', stats=get_stats())


@app.route('/fail')
def fail():
    return render_template('fail.html', stats=get_stats())


@app.route('/result')
def result():
    return render_template('result.html', stats=get_stats())


@app.route('/refund')
def refund():
    return render_template('refund.html', stats=get_stats())


@app.route('/chargeback')
def chargeback():
    return render_template('chargeback.html', stats=get_stats())
...
def reviews():
    fake_reviews = [
        ...
        },
        {
            "name": "Kirill7779991",
            "avatar": "https://vk.com/images/camera_200.png",
            "text": "Отличный сервер, без лагов и отзывчивый персонал 💪",
            "date": "1 октября 2025"
        }
    ]
